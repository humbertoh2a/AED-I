...
*/
int busca_atomo(node *n, int x, int nivel) {
    if (n == NULL) return -1;
    ...
    if (n->type == ATOM) {
        ...
        int res_interno = busca_atomo(n->head.list, x, nivel + 1);
        if (res_interno != -1) {
            return res_interno;
        }
    }

    return busca_atomo(n->tail, x, nivel);
}
...
int main()
{
  ...
  {
    nivel = busca_atomo(root->head.list, atomo, 0);
  }
